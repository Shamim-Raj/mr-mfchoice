<div class="product-card">
    ...
    <div class="product-card-details">
        ...
        @if(hasPromotion($product->id))
            <span class="price">{{ currency(promotionPrice($product->id),2) }}</span>
            <span class=""><del class="text-secondary">{{ currency($product->unit_price,2) }}</del> <small class="text-secondary">{{__('-')}}{{ round((($product->unit_price-promotionPrice($product->id))/$product->unit_price) *100) }}{{__('%')}}</small></span>
        @else
            @if($product->discount > 0)
                <span class="price">{{ currency(($product->sale_price),2) }} </span>
                <span><del class="text-secondary">{{ currency($product->unit_price,2) }}</del> <small class="text-secondary">{{__('-')}}@if($product->discount_type=='percentage'){{$product->discount}}@elseif($product->discount_type=='fixed'){{ round(($product->discount/$product->unit_price) *100) }} @endif{{__('%')}}</small></span>
            @else
